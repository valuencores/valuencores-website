/* ══════════════════════════════════════════════════════════════
   VALUENCORES — MAIN JS
   Scroll-reveal · Navbar state · Contact form feedback · Scroll Nav
══════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════
   SCROLL NAVIGATOR — Top / Down arrows + progress bar
══════════════════════════════════════════════════════════════ */
(function initScrollNav() {
  ...
  if (!nav || !btnTop || !btnDown) return;

  /* 스크롤이 40px 이상 내려가면 네비게이터 표시 */
  function updateNav() {
    ...
    /* 네비 표시 */
    nav.classList.toggle('visible', scrollY > 40);

    /* 진행바 */
    if (progressBar) progressBar.style.height = progress + '%';

    /* 최상단 근처이면 Top 버튼 숨김 */
    btnTop.classList.toggle('hidden', scrollY < 80);

    /* 최하단 근처이면 Down 버튼 숨김 */
    const nearBottom = scrollY >= docH - 40;
    btnDown.classList.toggle('hidden', nearBottom);
  }
